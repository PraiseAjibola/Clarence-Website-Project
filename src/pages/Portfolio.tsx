...
<section className="px-[8%] pt-[160px] md:pt-[200px] text-white bg-black">
  <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-10 items-center max-w-7xl mx-4">
    {/* Left Text */}
    <div className="md:col-span-2">
      <h2 className="text-4xl md:text-5xl font-bold mb-4">Portfolio</h2>
      <p className="text-white/70 leading-relaxed max-w-md">
        Lorem ipsum dolor sit amet consectetur. Sed molestie odio lectus
        venenatis proin nunc donec venenatis. Egestas enim a quam sed magna
        quis vehicula.
      </p>
    </div>

    {/* Stats */}
    <div className="flex flex-col sm:flex-row md:flex-col lg:flex-row md:col-span-2 gap-8 md:justify-end">
      <div>
        <h3 className="text-4xl md:text-5xl font-bold">
          <span className="text-white">50M</span>
          <span className="text-[#4ecdc4]">+</span>
        </h3>
        <p className="text-white/70 text-sm mt-1">Capital Investments</p>
      </div>
      <div>
        <h3 className="text-4xl md:text-5xl font-bold">
      ...
      {/* Portfolio Section */}
      <section className="px-[8%] py-16 md:pt-[200px]  pt-[160px]">
        {/* Filter Buttons */}
        <motion.div
...
<div className="relative z-[1]">
  <div className="grid gap-4 pt-12 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
    {filteredItems.map((item, index) => (
      <Link key={item.id} to={item.link}>
        ...
        >
          ...
          <div className="p-6 bg-black">
            ...
            <p className="text-white/50 text-xs md:text-sm">{item.year}</p>
          </div>
        </motion.div>
      </Link>
    ))}
      ...
      <section className="px-[8%] py-20 border-t border-[#333] text-center">
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12 max-w-6xl mx-auto">
          {[
            ...
            >
              <div className="text-[50px] font-bold text-[#4ecdc4] mb-2">
                {stat.number}
              </div>
      ...
      <div className="px-4 md:px-[8rem] mt-24 mb-24">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
          className="bg-gradient-to-br from-[#4ecdc4]/10 to-[#4ecdc4]/5 rounded-2xl p-12 md:p-16 text-center"
        >
          <h2 className="text-[36px] md:text-[48px] leading-[1.2] mb-6 font-bold">
            Ready to Partner With Us?
          </h2>
          <p className="text-[18px] leading-[1.6] text-white/70 max-w-[600px] mx-auto mb-8">
